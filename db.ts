...
/* 
FOR THE PHOTO SUBFOLDER -- FETCH ALL THE IMPORTANT INFROMATION
Similar to how get_articles_by_query only fetches around 10
articles for the home page, or whatever requires querying,
this will fetch images for the photos section 
(like 10 - 20)
*/
async function get_photos_by_article_extra(
  num?: number,
): Promise<ReceivedPhotoExtra[]> {
  const db = (await clientPromise).db();
  let article_extra_collection = await db.collection("article_extras");

  const limit = num || 20;

  /*
	Look up the photos from the article extra collection in:

	//sorted order from volume, issue, and rank
	That have the property type equaling to "image"

	Then with their fields, look up the article it is
	assosiated with AND

	Look up the contributors it is assosiated with
  */
  let photos = await article_extra_collection
    .aggregate<ReceivedPhotoExtra>([
      { $sort: { volume: -1, issue: -1, rank: -1 } },
      {
        $match: {
          type: "image",
...
//GET COVER IMAGE PHOTOS
// THIS DIFFERS BECAUSE IT MATCHES BASED ON
// IF A COVER IMAGE EXISTS ON AN ARTICLE.
async function get_photos_by_cover_image(
  num?: number,
): Promise<ReceivedCoverImage[]> {
  const db = (await clientPromise).db();
  let article_collection = await db.collection("articles");
  const limit = num || 20;

  let photos = await article_collection
    .aggregate<ReceivedCoverImage>([
      { $sort: { volume: -1, issue: -1, rank: -1 } },
      {
        $match: {
      ...
      { $unset: ["text", "is_published", "section_id", "created_at"] }
    ])
    .limit(limit)
    .toArray();
...
async function get_editors(): Promise<ReceivedEditor[]> {
  ...
  // with
  let editors = (await editors_collection
    .aggregate([
      {
        $lookup: {
          from: "staffs",
