...
import {
  ...
  ReceivedEditor,
  ReceivedRecruitmentsArrayDisplay,
  Department,
...
//Get all the recruitment material from "misc" > "type: 'recruitment material'"
async function get_recruitment_materials(): Promise<
  ReceivedRecruitmentsArrayDisplay[]
> {
  const db = (await clientPromise).db();
  let recruitments_collection = db.collection("misc");
  let recruitment_material = (await recruitments_collection
    .aggregate([
      { $match: { type: "recruitment_material" } },
      {
        $project: {
          _id: 0,
        },
    ...
    .toArray()) as ReceivedRecruitmentsArrayDisplay[];
  return recruitment_material;
}
