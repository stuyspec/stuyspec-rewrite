...
export default function MixedArticleDisplay(props: {
  ...
  const handle_load_more = async () => {
    if (props.additional_article_function) {
      ...
      const articles_with_photos = articlesProcessed.articlesWithPhotos.concat(
  ...
  const handle_paginate = async () => {
    if (props.additional_article_function) {
      ...
      const articles_with_photos = grouping.withPhotos
        .slice(0)
        .slice(0, image_articles);

      const articles_without_photos = grouping.withoutPhotos.concat(
        grouping.withPhotos.slice(0).slice(image_articles),
      );
  ...
  useEffect(() => {
    window.innerWidth <= 940 && handle_paginate();
  }, [page]);
    ...
    <div id={styles.mixed_article_view_container}>
      <div id={styles.mixed_article_view}>
              ...
              <Fragment key={index}>
                ...
                {index % ad_spacing == 0 ? (
                  <div className={styles.ad_parent}>
                    <MixedAdvertisment index={index / ad_spacing} />
                  </div>
                ) : (
                  <></>
                )}
        ...
        <section id={styles.top_mobile}>
          {articlesProcessed.sortedArticles.slice(0).map((article, index) => (
            <Fragment key={index}>
              <CenterArticle
                article={article}
                display_image
              ...
              {index % ad_spacing == 1 && index != 0 ? (
                <div className={styles.ad_parent}>
                  <MixedAdvertisment index={Math.floor(index / ad_spacing)} />
                </div>
              ) : (
                <></>
              )}
